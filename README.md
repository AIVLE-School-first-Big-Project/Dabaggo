...
1. 호스팅 된 주소로 접속하여 자체번역 모델 사용시 크롬-> 사이트 설정-> 안전하지 않은 컨텐츠를 허용으로 변경해주세요! (채팅 기반의 웹서비스 이므로 매우 권장함!!)
2. 1번의 방법이 안될 시 VSCode에서 "Live Server" 확장자 프로그램을 설치한 후 우측 하단에 "Go Live!" 버튼을 클릭하여 서비스를 이용해주세요! (권장함 !)
3. 로컬에서 Flask Server를 동작하고 싶을 시 Flask Host를 local로 변경해주시고, chatroom.html의 1228라인, 1249라인을 local로 변경해주세요!! (매우 권장하지 않음)
