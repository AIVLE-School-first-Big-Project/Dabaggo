...
>'Dabaggo'는 외국인과 소통을 위한 웹 동시 통역기 서비스입니다. <br>  https://carot-4cbc3.web.app/ 해당 URL 통해 접속가능(서버 구동시) <br> *자체번역 모델 사용시 크롬-> 사이트 설정-> 안전하지않은 컨텐츠를 허용으로 변경해주세요!<br>(처음에 2~3초 정도 번역시간 걸립니다)

## 사용 전 필독 !! (위에서 순서대로 따라해주세요!!!!)
1. 호스팅 된 주소로 접속하여 자체번역 모델 사용시 크롬-> 사이트 설정-> 안전하지 않은 컨텐츠를 허용으로 변경해주세요! (채팅 기반의 웹서비스 이므로 매우 권장함!!)<br>
2. 1번의 방법이 안될 시 VSCode에서 "Live Server" 확장자 프로그램을 설치한 후 우측 하단에 "Go Live!" 버튼을 클릭하여 서비스를 이용해주세요! (권장함 !)<br>
3. 로컬에서 Flask Server를 동작하고 싶을 시 Flask Host를 local로 변경해주시고, chatroom.html의 1228라인, 1249라인을 local로 변경해주세요!! (매우 권장하지 않음)<br>
