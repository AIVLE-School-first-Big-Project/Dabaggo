...
def eng_to_kor():
    if request.method=='POST':
        ...
        print('key:',keys[0])
        if keys[0]!='aivle':
            return 'reject(The key value is incorrect.)'
        else:
            src = args['text'][0]
            print('입력:',src)
            translation, attention = translate_sentence(src, SRC, TRG, model, device, logging=True)
            print("번역:", " ".join(translation))
            result=' '.join(translation).replace('<eos>','').replace('<unk>','')
            return result
@app.route('/home')
def home():
    return '영어를 한국어로 번역'
...
if __name__ == '__main__':
    ...
    SRC=joblib.load('model/SRC(en_to_ko).pkl')
    TRG=joblib.load('model/TRG(en_to_ko).pkl')
    model = torch.load('model/transformer_model(en_to_ko).pt',map_location=device)
    model.load_state_dict(torch.load('model/transformer_english_to_korean.pt',map_location=device))
